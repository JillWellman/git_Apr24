...
class State(Ruler):
	...
	def zsel_transform(self,dp):
		"""freeze  comes from two states bounding transition
		works off path state -- works --  freeze for now 0527"""
	...
	def store_state(self,dp):
		"""writes all state data"""
		if StatePath.LIST:
			hfile = self.parent.directory + '/hfile' + str(dp) + '.txt'
			with open(hfile, 'a') as f:	
				f.write(str(self.hueLst))

		# store_image(filename,save,image_name)
		self.ifile = self.parent.directory + '/ifile_' + str(dp) + '.png'
		...
		# self.lcstr= str(dp) + ','+ str(cx) + ',' + str(cy) + ',' + str(dw)
		
		# self.write_location_string(dp)

	def write_location_string(self,dp):
		file = self.parent.directory + '/lcfile.txt'

		if dp==0:
			with open(file, 'w') as f:
				f.write(str(self.lcstr) + '\n')
		else:
...
class StatePath(Ruler):
	...
	def draw_store(self,st,dp):
		...
		self.write_location_string(st)


	def write_location_string(self,st):   # in data file for whole state path
