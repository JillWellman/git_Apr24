...
from helpers import *
from histogram20 import HueGraph
...
class State(Ruler):
	...
	def __init__(self,parent,dp) -> None:
		...
		# self.image_file = 'data/image_fileB' + str(dp) + '.png'
		# self.text_file = 'data/text_fileB' + str(dp) + '.txt'
		# self.graph_file = 'data/graph_fileB' + str(dp) + '.png'
...
class StatePath(Ruler):
	...
	def zoom_loop(self):
		...
		im.graph_image = hg.color_frequency_graph(im.hueLst)
		...
		in_window(1.5*X+offset,Y/2,im.graph_image_filename,ip.woG.win)
		ip.bt = Button(ip.woG.win,Point(X - 60,Y - 30),60,30,'==>')
		ip.bt.draw()
